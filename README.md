...
| 自動更新 | 賽事期間每 10 分鐘，2026-07-21 後自動停用 |
...
`.github/workflows/worldcup-daily.yml` 每 10 分鐘跑一次 `--fetch` 並在資料變動時回推（GitHub 排程尖峰時可能延遲，實際約 10–20 分鐘）。需要在 repo 的 Settings → Secrets → Actions 新增 `FOOTBALL_DATA_TOKEN`。**沒設 secret 也不會讓 workflow 失敗**，只是不會更新賽果。2026-07-21（UTC）之後的第一次排程會自動停用本 workflow，不需人工收尾。
...
- 每秒重算倒數；每 2 分鐘於背景重抓 `data/matches.json`（靜默，僅在 console 記錄失敗）。
- 標頭顯示資料年齡（「3 分鐘前更新」）；超過 20 分鐘會轉為金色，提示排程可能延誤。
- 標頭的「台灣運彩官方盤口」是純外部連結（`target=_blank` + `rel=noopener noreferrer`）。
  本站**不抓取也不轉載**其賠率，理由見〈資料誠實性〉。
- 「重新載入」按鈕會回報結果：`更新中…` → `已載入新賽果` / `已是最新資料` / `更新失敗：<原因>`，5 秒後自動消失。
  按鈕抓的是 `data/matches.json` 這份由排程更新的快照，**不是直接打 football-data.org** —— 該 API 不回 CORS 標頭，
  且 token 不能放進公開頁面的 JS。要做到點擊即抓即時比分，需另建一個持有 token 的代理（如 Cloudflare Worker）。
...
- **不串接台灣運彩的即時盤口。** 技術上該站賽事頁是 JS 空殼，HTML 無賠率；官方公開端點
  `blob.sportslottery.com.tw/apidata/Live/On.json` 只列進行中賽事，不含世界盃賽前盤口。
  更根本的是，抓取並 republish 持牌博彩業者的盤口，授權狀況不明，且與本專案「不收錄博彩公司報價」
  的原則相牴觸。故僅在標頭提供官方連結，方便使用者自行前往查閱。
